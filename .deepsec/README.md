...
Currently configured project: `watch-movie` (target: `..`).
...
original init command. Checkpoints in `data/watch-movie/setup/setup-state.json`
...
`--project-id watch-movie` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/watch-movie/`.
