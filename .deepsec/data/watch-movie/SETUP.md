# Agent setup for `watch-movie`

This is a deepsec scanning workspace. Project `watch-movie` was just registered
(target: `..`). Setup is incomplete — `data/watch-movie/INFO.md`
...
2. **Fill in `data/watch-movie/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id watch-movie
pnpm deepsec process --project-id watch-movie
```
